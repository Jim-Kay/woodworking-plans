...
function calculateToteRackPlan(plan) {
  ...
  const tieRailPositions = [0, columns];
  ...
  const backBraceQty = columns >= 4 && rows >= 3 ? 2 : 0;
  ...
  const parts = [
    ...
    { part: 'Back diagonal 1x4 braces', qty: backBraceQty, length: backBraceLength, width: backBraceWidth, thickness: backBraceThickness, notes: backBraceQty ? 'Two rear chevron braces for wide racks; trim ends to fit and fasten each end into a back post after the rack is square.' : 'Not required for small racks; add rear diagonal bracing on wider builds if the rack racks side-to-side.' },
...
function buildToteRackAssembly(model) {
  ...
  [
    ...
    model.tieRailPositions.forEach((i) => {
  ...
  if (model.backBraceQty) {
    ...
    const bottomY = postBottomY + model.rail;
