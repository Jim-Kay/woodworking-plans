...
export function generateTwoStepStoolDesign(scenario = {}) {
  ...
  const parts = [
    ...
    ...legParts(parameters, legX, frontLegY, backLegY),
    ...
    physicalPart('rail.left.side', 'Left side stretcher rail', parameters.material, [parameters.rail_thickness_in, parameters.depth_in - parameters.leg_depth_in * 2, parameters.rail_height_in], [-legX, 0, railZLower], 'rail', {
      component_id: 'geometry.linear_rail',
      cut: { length_in: parameters.depth_in - parameters.leg_depth_in * 2, width_in: parameters.rail_height_in, thickness_in: parameters.rail_thickness_in }
    }),
    physicalPart('rail.right.side', 'Right side stretcher rail', parameters.material, [parameters.rail_thickness_in, parameters.depth_in - parameters.leg_depth_in * 2, parameters.rail_height_in], [legX, 0, railZLower], 'rail', {
      component_id: 'geometry.linear_rail',
      cut: { length_in: parameters.depth_in - parameters.leg_depth_in * 2, width_in: parameters.rail_height_in, thickness_in: parameters.rail_thickness_in }
    })
  ];
  ...
  const joints = [
    ...
    joint('joint.left.rail', 'fastened', ['rail.left.side', 'leg.front.left', 'leg.back.left'], 'left side rail ties front and back legs together'),
    joint('joint.right.rail', 'fastened', ['rail.right.side', 'leg.front.right', 'leg.back.right'], 'right side rail ties front and back legs together')
  ];
...
function legParts(parameters, legX, frontLegY, backLegY) {
  return [
...
function buildSteps(parameters) {
  return [
    step('step.cut', 'Cut stool parts', ['tread.lower', 'tread.upper', 'leg.front.left', 'leg.front.right', 'leg.back.left', 'leg.back.right', 'rail.front.lower', 'rail.back.upper', 'rail.left.side', 'rail.right.side'], [
      `Cut two treads, two ${cleanNumber(parameters.lower_step_height_in)} in front legs, two ${cleanNumber(parameters.height_in)} in back legs, and four apron or stretcher rails.`,
      'Keep paired legs exactly the same length so the stool sits flat.'
    ]),
    step('step.layout', 'Mark rail and tread locations', ['leg.front.left', 'leg.front.right', 'leg.back.left', 'leg.back.right'], [
    ...
    step('step.assemble_frame', 'Assemble leg and rail frames', ['leg.front.left', 'leg.front.right', 'leg.back.left', 'leg.back.right', 'rail.front.lower', 'rail.back.upper', 'rail.left.side', 'rail.right.side'], [
      'Fasten the front legs together with the front apron, then fasten the back legs together with the back apron.',
      'Add side stretcher rails to tie the front and back leg frames together. Check for square before tightening fasteners.'
    ]),
    step('step.install_treads', 'Install step treads', ['tread.lower', 'tread.upper', 'leg.front.left', 'leg.front.right', 'leg.back.left', 'leg.back.right'], [
      'Fasten the lower tread to the front leg frame and supporting side rails.',
      'Fasten the upper tread to the back leg frame. Use glue and mechanical fasteners appropriate for a standing surface.'
    ]),
