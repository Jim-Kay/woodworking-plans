export const PLAN_CATALOG = [
  ...
  },
  {
    ...
    buildSteps: [
      ...
      { title: 'Cut and label the parts', stage: 2, instructions: ['Cut the vertical posts, full-width top and bottom frame rails, bottom depth tie rails, and tote runner rails from the generated cut list.', 'Label front and back posts by column position so each runner row lines up across the rack.', 'Mark every runner height on all posts before assembly; matching marks are easier to keep square than measuring each rail one at a time.', 'Pre-drill screw locations near rail ends to reduce splitting.'] },
      { title: 'Assemble the doubled bottom base', stage: 1, instructions: ['Build the front and back bottom rails as doubled 2x4 beams, keeping each stacked pair flush along the top and outside face.', 'Install the doubled bottom depth tie rails between the front and back beams at every post line.', 'Keep the outside face of each bottom depth tie aligned with the outside face of its post location.', 'Check the base for square by comparing diagonal measurements before adding posts.'] },
      { title: 'Stand the posts and add top rails', stage: 3, dimensionContext: 'post', instructions: ['Set each front and back post on top of the doubled bottom frame rails at the marked bay spacing.', 'Fasten through the bottom rail into each post, keeping the post wide face oriented like the reference diagram.', 'Lay the full-width front and back top rails flat across the post tops.', 'Clamp the top rails before fastening so the rack stays plumb and the front/back frames remain parallel.'] },
      { title: 'Install runner rails row by row', stage: 4, dimensionContext: 'runner', instructions: ['Install two front-to-back runner rails for each tote bay; do not share one runner between neighboring totes.', 'Place each runner directly against its post so the inside clear width follows the calculated tote neck clearance.', 'Work from the bottom row upward, checking that matching left and right runners are level front-to-back.', 'Drive screws diagonally from opposite sides where rails meet posts so fasteners do not collide.'] },
      { title: 'Test fit empty totes', stage: 6, dimensionContext: 'runner', instructions: ['Slide an empty tote into every bay before loading the rack.', 'The lid lip should sit on both runners, while the tapered neck below the lip clears the runner rails.', 'Push the tote gently left and right; it should not be able to drop between the runner rails.', 'If a tote binds, re-check lip width, lid side gap, and runner clear width before adding more fasteners.'] },
      { title: 'Add casters or anchors', stage: 5, instructions: ['If the rack needs to roll, mount locking casters at the front and non-locking casters at the back under the doubled bottom frame rails.', 'Keep caster plates fully supported by the doubled base and use all mounting holes.', 'If the rack will be stationary or tall, skip casters and anchor the rack to wall framing with anti-tip hardware.', 'Test movement or anchoring while the rack is empty.'] },
      { title: 'Final load and inspection', stage: 999, instructions: ['Load totes gradually, starting with the lower rows and heaviest totes near the bottom.', 'Confirm the rack does not rack side-to-side after loading each row.', 'Keep total weight within the capacity of the lumber, screws, casters, and floor surface.', 'Re-check screws after the first few uses, especially if the rack is rolled while loaded.'] }
    ],
