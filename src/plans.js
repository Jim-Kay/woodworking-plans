export const PLAN_CATALOG = [
  ...
  },
  {
    ...
    buildSteps: [
      { title: 'Cut and mark the parts', stage: 0, instructions: ['Cut posts, front rails, back rails, side depth rails, and shelf slats from the generated cut list.', 'Sort parts by shelf level and rail direction so the bottom shelf parts are easy to grab first.', 'Mark every shelf height on the posts as a batch before assembly; matching marks are easier to keep level than measuring each rail one at a time.'] },
      { title: 'Build the bottom shelf frame flat on the floor', stage: 2, instructions: ['Lay the bottom-most back rail, front rail, and left/right side depth rails on the ground in their final footprint.', 'Square the rectangle by checking diagonals before fastening the corners.', 'Fasten the bottom shelf rails together while the assembly is still flat and easy to clamp.'] },
      { title: 'Install the bottom shelf slats', stage: 3, instructions: ['Place the bottom shelf slats or decking on the completed bottom rail frame.', 'Fasten each slat to the supporting rails so the bottom shelf becomes a rigid platform.', 'Re-check that the base is square before using it to locate the posts.'] },
      { title: 'Attach the corner posts to the base', stage: 1, instructions: ['Stand the four corner posts on the outside corners of the bottom shelf frame.', 'Clamp or hold each corner post plumb, then fasten it to the bottom front/back rails and side depth rails.', 'Use the completed bottom shelf as the layout reference instead of trying to clamp all posts in free space.'] },
      { title: 'Start the second shelf level', stage: 2, instructions: ['At the next shelf-height marks, install the side depth rails that connect the outer corner posts first.', 'Install the front rail sections for the same shelf level, using the side rails to keep the corner posts held at the correct depth.', 'Add the non-corner front and back posts against the front rail locations, then install the remaining side depth rails for that level.'] },
      { title: 'Deck the second shelf level', stage: 3, instructions: ['Place the second shelf slats after the second-level rails and intermediate posts are fastened.', 'Fasten the slats to the front/back rails and side depth rails so the shelf level locks the frame square.', 'Check the posts for plumb and the shelf for level before building higher.'] },
      { title: 'Repeat shelf-by-shelf upward', stage: 3, instructions: ['For each remaining shelf, repeat the same order: side depth rails at the corner posts, front/back rails, non-corner posts, remaining side depth rails, then shelf slats.', 'Work one shelf level at a time so each completed shelf stiffens the assembly before the next level is added.', 'Keep checking level, plumb, and square as the shelf gets taller.'] },
      { title: 'Final fasteners and anchoring', stage: 4, instructions: ['Add any remaining screws at rail-to-post and slat-to-rail joints.', 'Install wall anchors or anti-tip hardware if the shelf will carry tall or heavy loads.', 'Load the shelf gradually from the bottom up and recheck fasteners after the first use.'] }
    ],
