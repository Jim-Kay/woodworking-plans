export const PLAN_CATALOG = [
  ...
  },
  {
    ...
    buildSteps: [
      ...
      { title: 'Assemble the front and back frames', stage: 1, dimensionContext: 'post', images: [{ type: 'front-frame', phase: 1 }, { type: 'front-frame', phase: 2 }, { type: 'front-frame', phase: 3 }], instructions: ['Build the front frame flat on the floor with {postCountPerFrame} posts standing on the first bottom frame rail.', 'Use {frameRailLength} top and bottom frame rails, with {postClearWidth} clear between each pair of posts.', 'Screw through the first bottom rail into each post, then screw down through the top rail into each post.', 'After those joints are fastened, add the second bottom rail under the first and screw upward through it into the first rail.', 'Build the back frame the same way, checking that matching post positions and runner-height marks line up with the front frame.'] },
