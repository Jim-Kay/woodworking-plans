export const PLAN_CATALOG = [
  ...
  },
  {
    ...
    buildSteps: [
      ...
      { title: 'Assemble the front and back frames', stage: 1, dimensionContext: 'post', instructions: ['Build the front frame flat on the floor with {postCountPerFrame} posts standing on the doubled bottom frame rail and the top rail resting across the post tops.', 'Use {frameRailLength} top and bottom frame rails, with {postClearWidth} clear between each pair of posts.', 'Drive two screws at each post-to-bottom-rail and post-to-top-rail joint.', 'Build the back frame the same way, checking that matching post positions and runner-height marks line up with the front frame.'] },
      { title: 'Join the frames with doubled depth ties', stage: 2, instructions: ['Stand the front and back frames parallel to each other.', 'Install the doubled bottom depth tie rails between the frames at every post line.', 'Keep the outside face of each bottom depth tie aligned with the outside face of its post.', 'Fasten the doubled depth ties together and then into the front and back posts before checking the rack for square.'] },
      { title: 'Install runner rails row by row', stage: 3, dimensionContext: 'runner', instructions: ['Install two front-to-back runner rails for each tote bay; do not share one runner between neighboring totes.', 'Place each runner directly against its post so the inside clear width follows the calculated tote neck clearance.', 'Work from the bottom row upward, checking that matching left and right runners are level front-to-back.', 'Drive screws diagonally from opposite sides where rails meet posts so fasteners do not collide.'] },
      { title: 'Test fit empty totes', stage: 5, dimensionContext: 'runner', instructions: ['Slide an empty tote into every bay before loading the rack.', 'The lid lip should sit on both runners, while the tapered neck below the lip clears the runner rails.', 'Push the tote gently left and right; it should not be able to drop between the runner rails.', 'If a tote binds, re-check lip width, lid side gap, and runner clear width before adding more fasteners.'] },
      { title: 'Add casters or anchors', stage: 4, instructions: ['If the rack needs to roll, mount locking casters at the front and non-locking casters at the back under the doubled bottom frame rails.', 'Keep caster plates fully supported by the doubled base and use all mounting holes.', 'If the rack will be stationary or tall, skip casters and anchor the rack to wall framing with anti-tip hardware.', 'Test movement or anchoring while the rack is empty.'] },
